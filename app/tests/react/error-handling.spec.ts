...
describe("react/error handling", () => {
  for (const version of [16, 17, 18]) {
    ...
    it("shows error when component is missing before update", async (ctx) => {
      const { appDir, controller } = await ctx.setupTest(`react${version}`, [
        reactPlugin,
      ]);
      await appDir.update("src/App.tsx", {
  ...
  it("fails correctly when encountering broken SASS", async (ctx) => {
    const { appDir, controller } = await ctx.setupTest("react-sass", [
      reactPlugin,
    ]);
    await controller.show("src/App.tsx:App");
    const previewIframe = await controller.previewIframe();
